...
public class ErrorHandleControllerAdvice {
	 ...
	 * エラーハンドリング処理
	 *
	 * <p>例外発生情報(Exception)を取得してエラーログをlogファイルに出力後、エラー画面へ遷移させる<br>
	 ...
	 *
	 * <p>例外発生情報(MaxUploadSizeExceededException)を取得し、エラー画面へ遷移させる<br>
	...
	public ModelAndView maxUploadSizeExceededExceptionHandler(Exception e) {
		...
		//HttpStatusを416に設定し、Statusコード,Statusメッセージを取得
		HttpServletResponse response = ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getResponse();
		response.setStatus(HttpStatus.REQUESTED_RANGE_NOT_SATISFIABLE.value());
		String httpStatus = String.valueOf(response.getStatus());
		String httpStatusMessage = HttpStatus.REQUESTED_RANGE_NOT_SATISFIABLE.name();
